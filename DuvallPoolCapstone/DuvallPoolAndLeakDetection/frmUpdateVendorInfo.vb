Public Class frmUpdateVendorInfo
	'variable for intCustomerID, restricted to this form
	Dim receiveVendorID As Integer

	Public Sub New(ByVal passedVendorID As Integer)

		'Receive current intCustomerID from whichever form called this one
		InitializeComponent()
		receiveVendorID = passedVendorID

	End Sub


	Private Sub frmUpdateVendorInfo_Load(sender As Object, e As EventArgs) Handles MyBase.Load
		...
		Try
			...
			'Select statement
			strSelect = "SELECT * FROM TVendors WHERE intVendorID = " & receiveVendorID
			...
			'populate text boxes
			txtVendorName.Text = dt.Rows(0).Item(1).ToString
			txtContactName.Text = dt.Rows(0).Item(2).ToString
	...
	Private Sub btnSubmit_Click(sender As Object, e As EventArgs) Handles btnSubmit.Click
		...
		Try
			...
			If Validation() = True Then
				...
				strUpdate = "UPDATE TVendors " &
					"SET strVendorName = '" & strVendorName & "', " &
						...
						"strEmail = '" & strEmail & "' " &
					"WHERE intVendorID = " & receiveVendorID
				...
				If intRowsAffected > 0 Then
					MessageBox.Show("Vendor info has been updated")
					Me.Close()
				End If
